...
class GameLogic:
    def __init__(self, code, usable_colors, active_user, cheat, colors, guessed_colors, guesses_used, correct_guesses):
    ...
    def update(self, inputs):
        ...
        if self.won is True:
            ...
            print(self.get_guesses_used())
            print(self.get_guessed_colors())
            print(self.get_correct_guesses())
            return render_template("Game.jinja", form=self.form, colors=self.colors_info,
                                   code=self.mystery_code, cheat=self.cheat, rows=self.get_guesses_used(),
    ...
    def updateDb(self, win):
        ...
        userstats = UserStats(date_played=datetime.now(), win=win, cheat=self.cheat,
                              amount_of_guesses=self.guesses_used + 1, user_id=self.active_user)
